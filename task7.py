...
number = """select Product, Quantity from sales_dataa group by Product order by Quantity desc"""
dfff = pd.read_sql_query(number,a)
print(dfff)

# ploting graph
dfff.plot(kind="line",x='Product',y='Quantity',marker='o',legend=False)
plt.title("total quantity of product")
plt.xlabel("Product")
plt.ylabel("Quantity")
plt.tight_layout()
